        ...
        try {
            ...
            if (!args.length) {
                await kaya.sendMessage(
                    from,
                    { text: `❌ Usage: \`${prefix}play <nom de la musique>\`` },
                    { quoted: mek }
                );
            ...
            if (!data?.status || !data.audio) {
                await kaya.sendMessage(from, { text: "❌ Échec de la récupération API. Réessaie plus tard." }, { quoted: mek });
                await kaya.sendMessage(from, { react: { text: "❌", key: mek.key } });
                return;
            }
            ...
            if (error.code === "ENOTFOUND") errorMessage = "❌ Erreur réseau. Vérifie ta connexion.";
            else if (error.response?.status === 404) errorMessage = "❌ Musique indisponible.";
            else if (error.response?.status === 429) errorMessage = "❌ Trop de requêtes. Attends un instant.";
